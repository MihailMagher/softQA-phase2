...
# Directories
$INPUT_DIR = ".\login\transaction_terminal_input"
$OUTPUT_DIR = ".\login\transaction_terminal_output"
...
for ($i = 1; $i -le 9; $i++) {
    $num = "{0:D2}" -f $i
    $INPUT_FILE = Join-Path $INPUT_DIR "login$num.inp"
    $OUTPUT_FILE = Join-Path $OUTPUT_DIR "login$num.bto"

    Write-Output "Running test login$num..."
